...
if __name__ == '__main__':
    ...
    spark = connection.spark_conn(app="etlProduct",config=conf)

    path_query = os.getcwd()+'/Queries/'

    query = sqlparse.format(
    open(
        path_query+'TopProduct.sql','r'
    ...
    try:
        ...
        conn.commit()
        df.to_sql('topProduct', engine, if_exists='append', index=False)
        ...
        SparkDF.toPandas() \
                .to_csv(f"TopProduct.csv", index=False)
