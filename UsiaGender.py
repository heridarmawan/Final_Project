...
if __name__ == '__main__':
    ...
    spark = connection.spark_conn(app="etlUsia",config=conf)

    path_query = os.getcwd()+'/Queries/'

    query = sqlparse.format(
    open(
        path_query+'UsiaGender.sql','r'
    ...
    try:
        ...
        conn.commit()
        df.to_sql('UsiaGender', engine, if_exists='append', index=False)
        print(f"[INFO] Update Mart Success .....")

        #create CSV
        SparkDF = spark.createDataFrame(df)
        SparkDF.toPandas() \
                .to_csv(f"UsiaGender.csv", index=False)
